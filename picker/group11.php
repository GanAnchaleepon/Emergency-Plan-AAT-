...
// --- Configuration ---
$current_group_name = "GROUP 11"; // กำหนดกลุ่มสำหรับหน้านี้
...
/**
 * ค้นหางานตาม Part Number ที่สแกน ภายใน Rotation ปัจจุบัน
 * @param mysqli $conn Connection object
 * @param string $groupName ชื่อกลุ่ม
 * @param string $scannedPartNumber Part Number ที่สแกน (cleaned)
 * @param string $currentRotation Rotation ปัจจุบัน
 * @return array|null ข้อมูลงาน หรือ null ถ้าไม่พบ
 */
function findTaskByPartNumber($conn, $groupName, $scannedPartNumber, $currentRotation) {
    // ค้นหางานที่ Part Number ตรงกัน ใน Rotation ปัจจุบัน และยังไม่เสร็จ
    $stmt = $conn->prepare("
        SELECT * FROM picking_plans 
        WHERE group_name = ? 
        AND rotation = ? 
        AND REPLACE(UPPER(part_number), '-', '') = ? 
        AND status IN ('pending', 'inprogress') 
        ORDER BY position ASC 
        LIMIT 1
    ");
    $stmt->bind_param("sss", $groupName, $currentRotation, $scannedPartNumber);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getCurrentTask($conn, $groupName, $planIdSessionKey) {
    ...
    $currentRotation = getCurrentRotation($conn, $groupName);
    if (!$currentRotation) {
        return null; // ไม่มีงานเหลือเลย
    }
    
    // หางานแรกที่ยังไม่เสร็จใน Rotation ปัจจุบัน
    $stmt = $conn->prepare("SELECT * FROM picking_plans WHERE group_name = ? AND rotation = ? AND status IN ('pending', 'inprogress') ORDER BY position ASC LIMIT 1");
    $stmt->bind_param("ss", $groupName, $currentRotation);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($task = $result->fetch_assoc()) {
...
/**
 * แปลงหมายเลข Rotation เป็น 4 หลัก (1 -> 0001) สำหรับการแสดงผลเท่านั้น
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $task) {
    ...
    $scanned_data = trim($_POST['scanned_data'] ?? ''); // Scanned data from JS
    ...
    if ($submitted_step === $current_step) {
        ...
        switch ($current_step) {
            case 'scan_part_number':
                // แยก CSV: เอาเฉพาะคอลัมน์แรก (Part Number)
                $scan_parts = explode(',', $scanned_data);
                $part_number_from_scan = trim($scan_parts[0]);
                
                // ลบขีดออกและแปลงเป็นตัวพิมพ์ใหญ่
                $scanned_part_number_cleaned = strtoupper(str_replace('-', '', $part_number_from_scan));
                
                // หา Rotation ปัจจุบัน
                $currentRotation = getCurrentRotation($conn, $current_group_name);
                
                if (!$currentRotation) {
                    logActivity($conn, $plan_id, 'scan_part_number', $scanned_data, 'error');
                    $_SESSION['error_message'] = 'ไม่พบงานในระบบ';
                    break;
                }
                
                // ค้นหางานตาม Part Number ที่สแกนใน Rotation ปัจจุบัน
                $foundTask = findTaskByPartNumber($conn, $current_group_name, $scanned_part_number_cleaned, $currentRotation);
                
                if ($foundTask) {
                    // พบงานที่ตรงกับ Part Number ที่สแกน
                    logActivity($conn, $foundTask['id'], 'scan_part_number', $scanned_data, 'success');
                    
                    // อัปเดต session ให้ชี้ไปที่งานที่เจอ
                    $_SESSION[$plan_id_session_key] = $foundTask['id'];
                    
                    // อัปเดตสถานะเป็น inprogress ถ้ายังเป็น pending
                    if ($foundTask['status'] === 'pending') {
                        updatePickingPlanStatus($conn, $foundTask['id'], 'inprogress');
                    }
                    
                    $is_success = true;
                    $new_step = 'scan_item_lot';
                    $log_status = 'success';
                    $_SESSION['success_message'] = 'สแกน Part Number ถูกต้อง: ' . htmlspecialchars($scanned_data) . ' (Rotation: ' . formatRotation($foundTask['rotation']) . ', Position: ' . $foundTask['position'] . ')';
                } else {
                    // ไม่พบงานใน Rotation ปัจจุบัน
                    logActivity($conn, $plan_id, 'scan_part_number', $scanned_data, 'error');
                    $_SESSION['error_message'] = 'ไม่พบ Part Number นี้ใน Rotation ' . formatRotation($currentRotation) . ' หรืองานนี้ทำเสร็จแล้ว';
                }
                break;
            ...
            case 'scan_position':
                ...
                if ($scanned_data == $task['position']) { // ใช้ == เพราะอาจเป็น string vs int
                    ...
                    $_SESSION['error_message'] = 'ตำแหน่งที่สแกนไม่ถูกต้อง! สแกนได้: ' . htmlspecialchars($scanned_data) . ' (ที่ถูกต้องคือ: ' . htmlspecialchars($task['position']) . ')';
                }
                break;
        }
        ...
        header("Location: group11.php?group=" . urlencode($current_group_name));
        exit;
    } else {
...
<head>
    ...
    <style>
        ...
        .task-info-card .task-info-header {
            margin-bottom: 25px;
            padding-bottom: 20px;
            border-bottom: 2px solid #f0f0f0;
        }

        .task-info-card .task-info-header h2 {
            margin: 0;
            font-size: 1.4rem;
            font-weight: 700;
            color: #1a4f72;
            ...
            <div class="scan-form-container">
                <div class="step-progress-bar mb-4">
                    <div class="progress" style="height: 8px; border-radius: 10px; background: #e9ecef;">
                        <div class="progress-bar bg-primary" role="progressbar" 
                             style="width: <?php 
                                if ($current_step === 'scan_part_number') echo '25';
                                elseif ($current_step === 'scan_item_lot') echo '50';
                                elseif ($current_step === 'scan_rotation_label') echo '75';
                                elseif ($current_step === 'scan_position') echo '99';
                                else echo '100';
                             ?>%; transition: width 0.6s ease;"
                             aria-valuenow="<?php echo $current_step === 'scan_part_number' ? 25 : ($current_step === 'scan_item_lot' ? 50 : ($current_step === 'scan_rotation_label' ? 75 : ($current_step === 'scan_position' ? 99 : 100))); ?>" 
                             aria-valuemin="0" 
                             aria-valuemax="100">
                        </div>
                    </div>
                    <div class="text-center mt-2">
                        <small class="text-muted font-weight-bold">
                            <?php 
                                if ($current_step === 'scan_part_number') echo 'ขั้นตอน 1 จาก 4';
                                elseif ($current_step === 'scan_item_lot') echo 'ขั้นตอน 2 จาก 4';
                                elseif ($current_step === 'scan_rotation_label') echo 'ขั้นตอน 3 จาก 4';
                                elseif ($current_step === 'scan_position') echo 'ขั้นตอน 4 จาก 4';
                ...
                <form method="POST" action="" id="scanForm">
                    ...
                    <div class="row">
                        ...
                        <!-- Step 3: Scan Rotation Label -->
                        <div class="col-lg-3 col-md-12 mb-4">
                            <div class="step-card <?php echo $current_step === 'scan_rotation_label' ? 'active' : ($current_step !== 'scan_rotation_label' && $current_step !== 'scan_item_lot' && $current_step !== 'scan_part_number' && $current_step !== 'no_task' && $current_step !== 'display_summary' && $current_step !== 'group_complete' ? 'completed' : ''); ?>" 
                                 data-step="3">
                                <h4>
                                    <i class="fas fa-qrcode"></i> 
                                    <span>สแกน Rotation Label</span>
                                </h4>
                                <div class="form-group mb-0">
                                    <input type="text" 
                                           class="form-control scan-input" 
                                           id="scan_rotation_label_input" 
                                           placeholder="สแกน QR Code Rotation..."
                                           <?php echo $current_step !== 'scan_rotation_label' ? 'disabled' : ''; ?> 
                                           required>
                                </div>
                                <?php if ($current_step === 'scan_rotation_label'): ?>
                                <div class="mt-2">
                                    <small class="text-primary">
                                        <i class="fas fa-info-circle"></i> สแกน QR Code บน Dolly
                                    </small>
                                </div>
                                <?php endif; ?>
                            </div>
        ...
        <?php elseif ($current_step === 'display_summary'): ?>
            <div class="card">
                <div class="card-header bg-success text-white">
                    <h4><i class="fas fa-clipboard-check"></i> สรุป Rotation: <?php echo htmlspecialchars(formatRotation($_SESSION['current_rotation_summary'])); ?> (Dolly Summary)</h4>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            ...
                            foreach ($all_plans_for_group as $plan): 
                                if ($previous_rotation !== $plan['rotation']) {
                                    $rotation_class_index = 1 - $rotation_class_index;
                                    $previous_rotation = $plan['rotation'];
                                }
                            ?>
                                <?php if ($plan['status'] !== 'completed'): // แสดงเฉพาะ pending/inprogress ?>
                                <tr class="rotation-group-<?php echo $rotation_class_index; ?>">
                                    <td><?php echo htmlspecialchars(formatRotation($plan['rotation'])); ?></td>
                                    ...
                                    <td>
                                        <span class="badge badge-status badge-<?php echo $plan['status']; ?>">
                                            <?php
                                            switch ($plan['status']) {
                                                case 'pending': echo 'รอดำเนินการ'; break;
                                                case 'inprogress': echo 'กำลังดำเนินการ'; break;
                                                // case 'completed': echo 'เสร็จสมบูรณ์'; break;
                                                default: echo htmlspecialchars($plan['status']);
                                            }
                                            ?>
                                        </span>
                                    </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const stepInputMap = {
                ...
                'scan_rotation_label': 'scan_rotation_label_input',
