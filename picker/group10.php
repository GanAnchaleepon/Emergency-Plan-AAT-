...
// --- Configuration ---
$current_group_name = "GROUP 10"; // กำหนดกลุ่มสำหรับหน้านี้
...
/**
 * แปลงหมายเลข Rotation ให้เป็น 4 หลัก (เช่น 7 -> 0007) สำหรับการแสดงผลเท่านั้น
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $task) {
    ...
    if ($submitted_step === $current_step) {
        ...
        switch ($current_step) {
            case 'scan_part_number':
                ...
                if (!$currentRotation) {
                    ...
                    $_SESSION['error_message'] = 'ไม่พบงานที่ต้องทำ';
                    break;
                }
                ...
                if ($foundTask) {
                    ...
                    logActivity($conn, $plan_id, 'scan_part_number', $scanned_data, 'error');
                    $_SESSION['error_message'] = 'ไม่พบ Part Number: ' . htmlspecialchars($scanned_data) . ' ใน Rotation ปัจจุบัน (' . formatRotation($currentRotation) . ')';
                }
                break;
            ...
            case 'scan_item_lot':
                ...
                // *** ลบการตรวจสอบความยาว 12 ตัวอักษรออก ***
        ...
        header("Location: group10.php?group=" . urlencode($current_group_name));
        exit;
    } else {
