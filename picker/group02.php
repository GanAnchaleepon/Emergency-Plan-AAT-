...
// --- Configuration ---
$current_group_name = "GROUP 02"; // กำหนดกลุ่มสำหรับหน้านี้ // เปลี่ยนจาก "GROUP 01" เป็น "GROUP 02"
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $task) {
    ...
    if ($submitted_step === $current_step) {
        ...
        switch ($current_step) {
            case 'scan_part_number':
                ...
                if ($foundTask) {
                    ...
                    $log_status = 'success';
                    $_SESSION['success_message'] = 'สแกน Part Number ถูกต้อง: ' . htmlspecialchars($scanned_data) . ' (Rotation: ' . str_pad((string)intval($foundTask['rotation']), 4, '0', STR_PAD_LEFT) . ', Position: ' . $foundTask['position'] . ')';
                } else {
                    logActivity($conn, $plan_id, 'scan_part_number', $scanned_data, 'error');
                    $_SESSION['error_message'] = 'ไม่พบ Part Number: ' . htmlspecialchars($scanned_data) . ' ใน Rotation ปัจจุบัน (' . str_pad((string)intval($currentRotation), 4, '0', STR_PAD_LEFT) . ')';
                }
                break;
        ...
        header("Location: group02.php?group=" . urlencode($current_group_name)); // เปลี่ยนจาก group01.php เป็น group02.php
        exit;
    } else {
