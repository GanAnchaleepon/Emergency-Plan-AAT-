...
// --- Configuration ---
$current_group_name = "GROUP 09L"; // กำหนดกลุ่มสำหรับหน้านี้
...
/**
 * จัดรูปแบบหมายเลข Rotation ให้เป็น 4 หลัก (เช่น 1 -> 0001)
 * ใช้สำหรับแสดงผลเท่านั้น ไม่กระทบข้อมูลที่ใช้สร้าง QR หรือค้นหาในฐานข้อมูล
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $task) {
    ...
    if ($submitted_step === $current_step) {
        ...
        header("Location: group09l.php?group=" . urlencode($current_group_name));
        exit;
    } else {
            ...
            <div class="task-info-card">
                <div class="task-info-header">
                    ...
                    <?php if ($is_dummy_part): ?>
                        <span class="dummy-badge"><i class="fas fa-exclamation-triangle"></i> DUMMY</span>
                    <?php endif; ?>
                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3 col-md-6 mb-4">
                                <div class="step-card <?php echo $current_step === 'scan_rotation_label' ? 'active' : ($current_step !== 'scan_rotation_label' && $current_step !== 'scan_item_lot' && $current_step !== 'scan_part_number' && $current_step !== 'no_task' && $current_step !== 'display_summary' && $current_step !== 'group_complete' ? 'completed' : ''); ?>" 
                                    ...
                                    <div class="mt-2">
                                        <small class="text-primary">
                                            <i class="fas fa-info-circle"></i> สแกน QR code บน Label
                                        </small>
                                    </div>
                                    <?php endif; ?>
                                </div>
